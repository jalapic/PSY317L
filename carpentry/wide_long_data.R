...
df.wide <- data.frame(name = c("James", "Tyler", "Stephen", "Jennifer", "Carmen"),
...
df.long <- data.frame(name = c("James", "Tyler", "Stephen", "Jennifer", "Carmen"),
...
# `pivot_longer` makes the dataframes longer 
# `pivot_wider` makes the dataframes longer 
...
wheeldf %>% pivot_longer(cols = 3:6, names_to="time")  

wheeldf.long <- wheeldf %>% pivot_longer(cols = 3:6, names_to="time")  
...
wheeldf.long %>% 
  ...
  ggplot(aes(x=time,y=mean,group=strain, color=strain)) +
  geom_line(lwd=1) +
  xlab("day")+
